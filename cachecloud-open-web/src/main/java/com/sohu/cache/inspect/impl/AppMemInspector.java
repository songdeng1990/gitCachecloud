...
public class AppMemInspector extends BaseAlertService implements Inspector {
    ...
    private void alertAppMemUse(final AppDetailVO appDetailVO) {
        ...
        String content = String.format("应用(%s,%s)-内存使用率报警-预设百分之%s-现已达到百分之%s-请及时关注",
                appDesc.getAppId(), appDesc.getName(),appDesc.getMemAlertValue(), appDetailVO.getMemUsePercent());
    ...
    private void alertInstanceMemUse(final InstanceStats instanceStats, final AppDetailVO appDetailVO) {
        ...
        String content = String.format("分片(%s,%s,应用(%s))内存使用率报警-预设百分之%s-现已达到百分之%s-应用的内存使用率百分之%s-请及时关注",
                instanceInfo,
                appDetailVO.getAppDesc().getName(),
