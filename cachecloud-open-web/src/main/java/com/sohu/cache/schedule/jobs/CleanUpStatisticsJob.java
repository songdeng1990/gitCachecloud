...
public class CleanUpStatisticsJob extends CacheBaseJob {
    ...
    private static final String CLEAN_GROUP_HOUR_COMMAND_STATISTICS = "delete from group_hour_command_statistics where create_time < ?";

    private static final String CLEAN_GROUP_MINUTE_COMMAND_STATISTICS = "delete from group_minute_command_statistics where create_time < ?";

    private static final String CLEAN_GROUP_HOUR_STATISTICS = "delete from group_hour_statistics where create_time < ?";

    private static final String CLEAN_GROUP_MINUTE_STATISTICS = "delete from group_minute_statistics where create_time < ?";
    ...
    public void action(JobExecutionContext context) {
        ...
        try {
            ...
            cleanCount = jdbcTemplate.update(CLEAN_GROUP_HOUR_COMMAND_STATISTICS, time);
            logger.warn("clean_group_hour_command_statistics count={}", cleanCount);
            cleanCount = jdbcTemplate.update(CLEAN_GROUP_MINUTE_COMMAND_STATISTICS, time);
            logger.warn("clean_group_minute_command_statistics count={}", cleanCount);
            cleanCount = jdbcTemplate.update(CLEAN_GROUP_HOUR_STATISTICS, time);
            logger.warn("clean_group_hour_statistics count={}", cleanCount);
            cleanCount = jdbcTemplate.update(CLEAN_GROUP_MINUTE_STATISTICS, time);
            logger.warn("clean_group_minute_statistics count={}", cleanCount);
