...
public class AppStatsCenterImpl implements AppStatsCenter {
    ...
    public List<AppStats> getAppStatsListByMinuteTime(long appId, long beginTime, long endTime) {
        ...
        try {
            ...
            appStatsList = appStatsDao.getAppStatsList(appId, td);
            
            if (td.getDimensionality() == AppStatsDao.HOUR_DIMENSIONALITY){
            	for (AppStats as : appStatsList){
                	as.setUsedMemory(as.getUsedMemory()/60);
                }
            }
    ...
    public AppDetailVO getAppDetail(long appId) {
        ...
        ConfigTemplate configTemplate = instanceConfigDao.getTemplateById(appDesc.getTemplateId());
        resultVO.setTemplateName(configTemplate == null ? "" : configTemplate.getName());
        ...
        if(instanceStatsList != null && instanceStatsList.size() > 0){
            ...
            for (InstanceInfo instanceInfo : instanceList) {
                ...
                if (ipSets.add(ip)){
                	allMachineMaxmemory += machineInfo.getMem();
                }
        ...
        resultVO.setMachineNum(ipSets.size());
        
        //当某个应用的所有redis的maxmemory设置为0时，以所在机器的内存作为实例最大的内存上限
        if (allMachineMaxmemory == 0) {
        	resultVO.setMemUsePercent(0);
        }else{
        	if (allMaxMemory == 0){
        		allMaxMemory = allMachineMaxmemory*1024*1024;
        		resultVO.setMem(allMachineMaxmemory);
        	}
    ...
    @Override
    public List<QueneSizeStatus> getQueneSizeStatByMinuteTime(long beginTime, long endTime) {
        return appStatsDao.getQueneSizeList(beginTime,endTime);
    }
    ...
    public InstanceConfigDao getInstanceConfigDao() {
