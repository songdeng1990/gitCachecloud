...
public class AppController extends BaseController {
    ...
    @RequestMapping("/getMutiQueneSizeStats")
    public ModelAndView getMutiStatQueueSizeStats(HttpServletRequest request,
                                                  HttpServletResponse response, Model model, Long appId) throws ParseException {
        ...
        long beginTime = NumberUtils.toLong(DateUtil.formatYYYYMMddHHMM(startDate));
        long endTime = NumberUtils.toLong(DateUtil.formatYYYYMMddHHMM(endDate));
        List<QueneSizeStatus> queneSizeStatusList = appStatsCenter.getQueneSizeStatByMinuteTime(beginTime, endTime);
        if (queneSizeStatusList.size() > 0) {
            result = assembleMutiQueneSizeStatsJsonMinute(queneSizeStatusList, DateUtil.formatYYYY_MM_dd(startDate), startDate);
        }
        model.addAttribute("data", result);
        return new ModelAndView("");
    }

    @RequestMapping("/redictQueneJsp")
    public ModelAndView redictQueneJsp(HttpServletRequest request,
                                       HttpServletResponse response, Model model, Long appId) throws ParseException{
        String start_date = request.getParameter("startDate");
        String end_date = request.getParameter("endDate");
        if (StringUtils.isEmpty(start_date)){
            start_date = DateUtil.formatYYYY_MM_dd(new Date());
        }
        if (StringUtils.isEmpty(end_date)){
            end_date = DateUtil.formatYYYY_MM_dd(DateUtils.addDays(new Date(),1));
        }
        ...
        model.addAttribute("queueActive", SuccessEnum.SUCCESS.value());
        model.addAttribute("startDate", start_date);
        model.addAttribute("endDate",end_date);
        return new ModelAndView("manage/config/queneStat");
    }
    ...
    public ModelAndView doAppList(HttpServletRequest request,
        ...
        // 2.4 业务组
        List<BusinessGroup> groupList = appService.getAllBusinessGroup();
        model.addAttribute("appGroupList", groupList);
    ...
    public ModelAndView doAppAdd(HttpServletRequest request,
        ...
        if (appDesc != null) {
            ...
            appDesc.setBackupDays(ConstUtils.DEFALT_BAKCUP_DAYS);
            appDesc.setOpenAlarm(ConstUtils.DEFALT_OPEN_ALARM);
    ...
    public ModelAndView doChangeAppAlertConfig(HttpServletRequest request,
        ...
        int openAlarm = NumberUtils.toInt(request.getParameter("openAlarm"),1);
        logger.info("修改应用报警配置 openAlarm = {}",openAlarm);
        SuccessEnum result = appService.changeAppAlertConfig(appId, memAlertValue,clientConnAlertValue,openAlarm, getUserInfo(request));
    ...
    public ModelAndView doCleanAppData(HttpServletRequest request, HttpServletResponse response, Model model, long appId) {
        ...
        if (appId > 0) {
            ...
            if (checkAppUserProvilege(request, appId) && appDeployCenter.cleanAppData(appId, getUserInfo(request), errMsg)) {
                successEnum = SuccessEnum.SUCCESS;
            }
        }
        if (successEnum.value() == 1) {
            model.addAttribute("code",1);
        }else {
            model.addAttribute("errMsg",errMsg.toJSONString());
    ...
    private String assembleMutiQueneSizeStatsJsonMinute(List<QueneSizeStatus> appStats, String statName, Date startDate) {
        if (appStats == null || appStats.isEmpty()) {
            return "[]";
        }
        Map<String, List<HighchartPoint>> map = new HashMap<String, List<HighchartPoint>>();
            List<HighchartPoint> list = new ArrayList<HighchartPoint>();
            for (QueneSizeStatus stat : appStats) {
                try {
                    HighchartPoint highchartPoint = HighchartPoint.getFromQueneSizeStats(stat, startDate, 0);
                    if (highchartPoint == null) {
                        continue;
                    }
                    list.add(highchartPoint);
