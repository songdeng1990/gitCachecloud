...
public class ConfigManageController extends BaseController {
    ...
    @RequestMapping(value = "/updateQueueRadio")
    public ModelAndView updateQueueRadio(HttpServletRequest request, HttpServletResponse response, Model model){
        SuccessEnum successEnum;
        Map<String, String> configMap = new HashMap<String, String>();
        try {
            String ratio = request.getParameter("collect.queue.alert.ratio");
            configMap.put("collect.queue.alert.ratio",ratio);
            if (!isNumeric(ratio)){
                successEnum = SuccessEnum.FAIL;
            }else {
                successEnum = configService.updateConfig(configMap);
        ...
        model.addAttribute("success",successEnum.value());
        return new ModelAndView("redirect:/admin/app/redictQueneJsp");
    }
