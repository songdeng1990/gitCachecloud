...
public class InstanceManageController extends BaseController {
     ...
     * @return
     */
    @RequestMapping(value = "/batch/shutdownInstance")
    public ModelAndView batchShutdownInstance(HttpServletRequest request, HttpServletResponse response, Model model) {
        AppUser appUser = getUserInfo(request);
        List<Integer> ids = (List<Integer>) request.getSession().getAttribute("instanceIds");
        logger.warn("------user {} batch shutdownInstance {} ", appUser.getName(),ids);
        boolean result = true;
        boolean temp = false;
        String message = "";
        for (Integer id : ids) {
                try {
                   temp = instanceDeployCenter.shutdownExistInstance(id);
                } catch (Exception e) {
                    logger.error(e.getMessage(), e);
                   message = message + id + "下线异常,原因" + e.getMessage();
                }
                if (!temp){
                    result = temp;
                }
            logger.warn("user {} shutdownInstance {}, result is {}", appUser.getName(), id, result);
        }
     ...
     * @return
     */
    @RequestMapping(value = "/batch/startInstance")
    public ModelAndView batchStartInstance(HttpServletRequest request, HttpServletResponse response, Model model) {
        AppUser appUser = getUserInfo(request);
        List<Integer> ids = (List<Integer>) request.getSession().getAttribute("instanceIds");
        logger.warn("user {} batch startInstance {} ", appUser.getName(), ids);
        boolean result = true;
        boolean temp = false;
        String message = "";
        for (Integer id : ids) {
            try {
                temp = instanceDeployCenter.startExistInstance(id);
            } catch (Exception e) {
                logger.error(e.getMessage(), e);
                message = message + id + "启动异常,原因" + e.getMessage();
            }
            if (!temp){
                result = temp;
            }
            logger.warn("user {} startInstance {}, result is {}", appUser.getName(), id, result);
        }
        model.addAttribute("message", message);
        if (result) {
            model.addAttribute("success", SuccessEnum.SUCCESS.value());
    ...
    public ModelAndView doDeleteInstance(HttpServletRequest request, HttpServletResponse response, Model model, int instanceId) {
        AppUser appUser = getUserInfo(request);
        logger.warn("user {} deleteInstance {} ", appUser.getName(), instanceId);
        Result result = null;
        if (instanceId > 0) {
            try {
                result = instanceDeployCenter.deleteExistInstance(instanceId);
            } catch (Exception e) {
                logger.error(e.getMessage(), e);
                model.addAttribute("message", e.getMessage());
            }
        } else {
            logger.error("dodeleteInstance instanceId:{}", instanceId);
            model.addAttribute("message", "wrong param");
        }
        logger.warn("user {} deleteInstance {}, result is {}", appUser.getName(), instanceId, result);
        if (result.isSuccess()) {
            model.addAttribute("success", SuccessEnum.SUCCESS.value());
        } else {
            model.addAttribute("success", SuccessEnum.FAIL.value());
