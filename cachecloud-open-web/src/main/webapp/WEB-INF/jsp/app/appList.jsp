...
<head>
    ...
    function cleanAppData(appId){
        ...
        $.ajax({
            type: "get",
            url: "/admin/app/cleanAppData.json",
            data:
                {
                    appId: appId
                },
            success: function (data) {
                if(data.code == 1){
                    ...
                    alert("appId:" + appId + "清除数据失败,结果:" + data.errMsg);
                }
						...
						<div class="form-group">
							<select name="businessGroupId" class="form-control">
								...
								<c:forEach var="group" items="${appGroupList}">
									<option value="${group.businessGroupId}" <c:if test="${appSearch.businessGroupId == group.businessGroupId}">selected</c:if>>
	            ...
	            <tbody>
	            	<c:forEach items="${appDetailList}" var="appDetail">
	            		 <tr>
		                    ...
		                    <td>
		                    	<c:choose>
		                    		<c:when test="${appDetail.appDesc.needPersistence == 1}">是</c:when>
		                    		<c:otherwise>否</c:otherwise>
		                    	</c:choose>
		                    </td>
							 <td>
								 <a target="_blank" href="/admin/group/index.do?groupId=${appDetail.appDesc.businessGroupId}">${appDetail.appDesc.businessGroupName}</a>
							 </td>
		                    <td>
								<c:choose>
									<c:when test="${appDetail.appDesc.status == 2 or appDetail.appDesc.status == 3 or appDetail.appDesc.status == 4}">
