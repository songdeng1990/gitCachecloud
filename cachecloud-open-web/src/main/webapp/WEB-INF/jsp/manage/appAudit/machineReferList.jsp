...
<div class="row">
	<div class="col-md-12">
		<h3 class="page-title">机器列表</h3>
	</div>
	<div class="btn-group" style="float: right">
		<form action="/manage/app/initAppDeploy.do?appAuditId=${appAudit.id}"
			method="post" class="form-horizontal form-bordered form-row-stripped">
			<label class="control-label"> 机器ip: </label> &nbsp;<input type="text"
				name="ipLike" id="ipLike" value="${ipLike}" placeholder="iplike匹配" />
			<label class="control-label"> 额外说明: </label>
				...
				<table class="table table-striped table-bordered table-hover"
					...
					<tbody>
						<c:forEach items="${machineList}" var="machine">
							<tr class="odd gradeX">
								<td><a target="_blank"
									href="/manage/machine/machineInstances.do?ip=${machine.ip}">${machine.ip}</a>
								</td>
								<td>${machine.info.extraDesc}</td>
								<td><span style="display: none"><fmt:formatNumber
											value="${machine.memoryUsageRatio / 100}" pattern="0.00" /></span>
								...
								<td><fmt:formatNumber var="fmtMemoryAllocatedRatio"
										value="${((machine.memoryAllocated)/1024)*100.0/(machine.memoryTotal/1024/1024/1024)}"
										pattern="0.00" /> <span style="display: none"><fmt:formatNumber
									...
									<div class="progress margin-custom-bottom0">
										...
										<label style="color: #000000"> <fmt:formatNumber
												value="${((machine.memoryAllocated)/1024)}" pattern="0.00" />G&nbsp;&nbsp;Used/
											<fmt:formatNumber
												value="${ machine.memoryTotal/1024/1024/1024}"
												pattern="0.00" />G&nbsp;&nbsp;Total
										</label>
								...
								<td><fmt:formatNumber
										value="${machine.traffic / 1024 / 1024}" pattern="0.00" />M</td>
								<td>${machine.load}</td>
								<td><c:choose>
